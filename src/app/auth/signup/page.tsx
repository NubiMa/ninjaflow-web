...
import { Mail, Lock, Eye, EyeOff, User, Zap, ArrowLeft } from "lucide-react";
...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try { await signUpWithEmail(email, password, name); }
      ...
      if (err?.code === "auth/email-already-in-use") setError("Email sudah digunakan. Coba login.");
  ...
  const handleGoogle = async () => {
    setError(""); setLoading(true);
    try { await signInWithGoogle(true); }
    catch { setError("Pendaftaran Google gagal."); }
    finally { setLoading(false); }
  };
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#0B1215", display: "flex" }}>
      {/* Left Panel */}
      <div className="hidden md:flex" style={{ width: "50%", position: "relative", overflow: "hidden", background: "linear-gradient(135deg, #0D1A1F, #152129)", alignItems: "center", justifyContent: "center", padding: 48 }}>
        <div style={{ position: "absolute", top: "20%", left: "10%", width: 280, height: 280, borderRadius: "50%", background: "rgba(79,209,197,0.06)", filter: "blur(80px)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: "15%", right: "5%", width: 180, height: 180, borderRadius: "50%", background: "rgba(125,211,252,0.05)", filter: "blur(60px)", pointerEvents: "none" }} />
        <div style={{ position: "relative", zIndex: 1, maxWidth: 400 }}>
          ...
          <h2 style={{ fontSize: 32, fontWeight: 800, color: "#F5F7FA", lineHeight: 1.25, marginBottom: 16, letterSpacing: "-0.03em" }}>
            Mulai perjalanan<br />finansialmu<br />
            <span style={{ background: "linear-gradient(135deg, #4FD1C5, #7DD3FC)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>hari ini.</span>
          </h2>
          <p style={{ fontSize: 15, color: "#748391", lineHeight: 1.6, marginBottom: 36 }}>Gratis selamanya. Tanpa kartu kredit.</p>
          {[
            { emoji: "🤖", text: "AI reflection personal setiap minggu" },
            { emoji: "📸", text: "Scan struk otomatis dengan kamera" },
            { emoji: "📊", text: "Insight spending pattern yang mudah dipahami" },
            { emoji: "🔒", text: "Data terenkripsi dan aman" },
          ].map(({ emoji, text }, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              <span style={{ fontSize: 18, flexShrink: 0 }}>{emoji}</span>
              <span style={{ fontSize: 13.5, color: "#AAB7C2" }}>{text}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "32px 24px" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#F5F7FA", marginBottom: 6, letterSpacing: "-0.02em" }}>Buat Akun</h1>
          <p style={{ fontSize: 14, color: "#748391", marginBottom: 28 }}>Sudah punya akun? <Link href="/auth/login" style={{ color: "#4FD1C5", textDecoration: "none", fontWeight: 500 }}>Masuk</Link></p>
          ...
          <form onSubmit={handleSignup} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ position: "relative" }}>
              <User size={15} color="#748391" style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)" }} />
              <input required value={name} onChange={e => setName(e.target.value)} placeholder="Nama lengkap" className="input-field" style={{ paddingLeft: 40 }} />
            </div>
            <div style={{ position: "relative" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input type={showPw ? "text" : "password"} required value={password} onChange={e => setPassword(e.target.value)} placeholder="Password (min. 6 karakter)" className="input-field" style={{ paddingLeft: 40, paddingRight: 44 }} />
          ...
          <p style={{ fontSize: 11, color: "#748391", textAlign: "center", marginTop: 20 }}>
            Dengan mendaftar, kamu setuju dengan <span style={{ color: "#4FD1C5" }}>Syarat & Ketentuan</span> kami.
          </p>
        </motion.div>
      </div>
    </div>
